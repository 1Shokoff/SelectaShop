...
def main() -> int:
    ...
    # 0600 на момент создания: в файле ключ подписи сессий, пароли БД и
    # ключ шифрования персональных данных.
    #
    # Права будут ослаблены до 0640 командой `make config`: этот же файл
    # читает пользователь приложения внутри контейнера (uid 10001), и с
    # правами 0600 контейнер уходит в перезапуск. Подробности и вторая
    # линия защиты — в scripts/fix_config_perms.sh.
    TARGET.chmod(0o600)

    # Каталог — граница доступа на хосте. Посторонний пользователь не
    # сможет войти в него, каким бы ни был режим самого файла.
